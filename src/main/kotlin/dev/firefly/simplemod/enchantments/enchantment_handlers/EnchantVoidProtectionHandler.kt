...
object EnchantVoidProtectionHandler : Listenable {
    ...
    fun onPlayerTick(e: TickEvent.PlayerTickEvent) {
        ...
        val level = getItemSpecificEnchantLevel(stack, EnchantVoidProtection.INSTANCE)
        val uuid = p.uniqueID
        if (level > 0) {
            if (p.posY <= -64 && p.fallDistance > 32.0) {
                val enchants = EnchantmentHelper.getEnchantments(stack)
                if (level - 1 > 0) {
                    enchants[EnchantVoidProtection.INSTANCE] = level - 1
                } else enchants.remove(EnchantVoidProtection.INSTANCE)
                p.tp(y=256.0)
